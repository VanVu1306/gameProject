...
void revealEmptyCells(vector<vector<char>>& minefield, vector<vector<bool>>& visited, int x, int y, int m, int n) {
    if (!isValid(x, y, m, n) || visited[x][y])
        return;

    visited[x][y] = true;

    if (countMines(minefield, x, y, m, n) == 0) {
        minefield[x][y] = '-';
        ...
        for (int i = 0; i < 8; i++) {
            int nx = x + dx[i];
            int ny = y + dy[i];
            revealEmptyCells(minefield, visited, nx, ny, m, n);
        }
    } else {
...
void playMineSweeper(int m, int n, int k) {
    ...
    while (!gameOver) {
        ...
        if (minefield[x][y] == '*') {
            ...
            revealEmptyCells(minefield, visited, x, y, m, n);
